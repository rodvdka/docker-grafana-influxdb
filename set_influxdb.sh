...
if [ -f "/.influxdb_configured" ]; then
    ...
    curl -XPOST $(echo ${API_URL}'/query') --data-urlencode "q=CREATE DATABASE data"

    echo "=> Creating User for database: data"
    curl -XPOST $(echo ${API_URL}'/query') --data-urlencode "q=CREATE USER \"data\" WITH PASSWORD 'data' WITH ALL PRIVILEGES"
